    ...
    const server = http.createServer(async (req, res) => {
        try {
            ...
            if (req.method === 'GET' && url.pathname === '/v1/stats') {
                ...
                client.stats(stream).then(r => send(res, 200, r)).catch(() => send(res, 500, { error: 'stats failed' }));
                return;
            }
            if (req.method === 'GET' && url.pathname === '/v1/metrics') {
                client.metrics().then(r => send(res, 200, r)).catch(() => send(res, 500, { error: 'metrics failed' }));
                return;
            }
