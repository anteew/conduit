...
let msgpack;
try {
  msgpack = await import('msgpackr');
} catch (e) {
  console.log('MessagePack not available:', e.message);
}
...
function benchmark(codec, data, iterations) {
...
function printResults(testName, payload, jsonResult, msgPackResult) {
  console.log(`\n${'='.repeat(70)}`);
...
if (msgpackCodec) {
  ...
  console.log('   • Install: npm install msgpackr');
