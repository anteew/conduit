...
function analyzeMemoryTrend(): { trend: string; growth: number; stable: boolean } {
  ...
  let trend = 'stable';
  if (growth > growthThreshold * 2.5) trend = 'leaking';
  else if (growth > growthThreshold) trend = 'growing';
  else if (growth < -10) trend = 'decreasing';
  
  return { trend, growth, stable };
}

async function httpClient(id: number, endTime: number): Promise<void> {
  let reqCount = 0;
...
async function wsClient(id: number, endTime: number): Promise<void> {
  return new Promise((resolve) => {
    const ws = new WebSocket(`ws://127.0.0.1:9088/v1/subscribe?stream=agents/SoakWS${id}/inbox`);
    let msgCount = 0;
    let creditTimer: NodeJS.Timeout;
...
async function uploadClient(id: number, endTime: number): Promise<void> {
  let uploadCount = 0;
...
(async () => {
  ...
  if (!overallPass) {
    console.log('\n⚠️  RECOMMENDATIONS:');
    if (!check1) console.log('  • Investigate low throughput - check CPU/network constraints');
    if (!check2) console.log('  • Review error logs and add retry logic');
    if (!check3) console.log('  • Profile memory usage - possible leak detected');
    if (!check4) console.log('  • Check WebSocket cleanup - connections not closing properly');
  }
