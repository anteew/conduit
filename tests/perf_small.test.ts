...
function calculateStats(latencies: number[]): LatencyStats {
  ...
  for (const lat of latencies) {
    if (lat < 1) histogram['<1ms']++;
    else if (lat < 5) histogram['1-5ms']++;
    else if (lat < 10) histogram['5-10ms']++;
    else if (lat < 20) histogram['10-20ms']++;
    else if (lat < 50) histogram['20-50ms']++;
    else if (lat < 100) histogram['50-100ms']++;
    else histogram['>100ms']++;
  }
...
async function sequentialTest(): Promise<void> {
  console.log(`\n${'='.repeat(60)}`);
