...
function httpPost(path: string, payload: any, contentType = 'application/json'): Promise<{ status: number; body: any }> {
  return new Promise((resolve, reject) => {
...
function createLargeObject(sizeBytes: number): any {
  const baseObj: any = { data: [] };
...
async function testWsSizeCap(): Promise<void> {
  return new Promise((resolve) => {
...
async function testWsDepthCap(): Promise<void> {
  return new Promise((resolve) => {
...
async function testWsValidPayload(): Promise<void> {
  return new Promise((resolve) => {
