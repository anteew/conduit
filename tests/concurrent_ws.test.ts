...
class ConcurrentWSTest {
  ...
  private memoryFinal = 0;

  async connectClient(clientId: number): Promise<{success: boolean; time: number}> {
  ...
  async enqueueMessages(count: number): Promise<void> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      promises.push(
        httpPost('http://127.0.0.1:9087/v1/enqueue', {
          to: stream,
          envelope: {
            id: `msg-${Date.now()}-${i}`,
  ...
  async grantCredits(): Promise<void> {
  ...
  async run(): Promise<TestReport> {
    ...
    try {
      ...
      const connectionPromises: Promise<{success: boolean; time: number}>[] = [];
