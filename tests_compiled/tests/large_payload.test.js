...
(async () => {
    ...
    try {
        ...
        class ZeroStream extends Readable {
            remaining;
            chunkSize;
            constructor(total, chunkSize = 1024 * 1024) { super(); this.remaining = total; this.chunkSize = chunkSize; }
            _read(_n) {
                if (this.remaining <= 0) {
                    this.push(null);
                    return;
        ...
        if (FILE_PATH) {
            try {
                ...
                console.log(`POST: using file ${FILE_PATH} (${(st.size / 1024 / 1024).toFixed(1)}MB)`);
            }
            catch { }
        }
        else {
        ...
        await new Promise((resolve, reject) => {
            ...
            if (FILE_PATH)
                ...
                new ZeroStream(sizeBytes).pipe(req);
        });
        ...
        if (SINK_PATH) {
            ...
            while (Date.now() - startWait < 60000) { // wait up to 60s
                try {
                    ...
                    if (st.size === expected) {
                        console.log(`VERIFY: sink size ok (${(st.size / 1024 / 1024).toFixed(1)}MB)`);
                        break;
                    }
                }
                catch { }
            ...
            if (final !== expected) {
                throw new Error(`Sink size mismatch: got ${final}, expected ${expected}`);
            }
        }
    }
    finally {
