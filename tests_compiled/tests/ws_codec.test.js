    ...
    if (!msgPackCodec) {
        ...
        try {
            ...
            ws.on('message', (data, isBinary) => {
                ...
                const msg = msgPackCodec.decode(Buffer.from(data));
                if (msg.deliver) {
...
console.log('[T7022-6] Testing frame encoding for both codecs...');
{
    ...
    if (!msgPackCodec) {
        ...
        try {
            ...
            wsJson.on('message', (data, isBinary) => {
                jsonBinary = isBinary;
            });
            wsMsgPack.on('message', (data, isBinary) => {
